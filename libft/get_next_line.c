...
static void	read_lines(int fd, char **saved, char **temp)
{
	...
	if (!buffer)
	{
		ft_free2(saved, temp, NULL);
		return ;
	}
	while (readed_bytes != 0 && !ft_strchr2(*saved, '\n'))
	{
		readed_bytes = read(fd, buffer, BUFFER_SIZE);
		if (readed_bytes == -1)
		{
			ft_free2(saved, temp, &buffer);
			return ;
		}
